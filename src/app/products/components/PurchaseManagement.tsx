                            ...
                            <TableBody>
                                {paginatedPurchases.map((purchase) => (
                                    <TableRow key={purchase.id}>
                                        <TableCell>{purchase.provider.id}</TableCell>
                                        <TableCell>{purchase.provider.name}</TableCell>
                                        <TableCell>{new Date(purchase.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }).replace(/ (\w+)/, (match, month) => ' ' + month.charAt(0).toUpperCase() + month.slice(1))}</TableCell>
                                        <TableCell>${calculateTotalCost(purchase).toFixed(2)}</TableCell>
                                        <TableCell>
                                            <div className="flex justify-center space-x-1">
                                                <Button size="sm" variant="light" onPress={() => handleViewDetail(purchase)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/>
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                                    </svg>
                                                </Button>
                                                <Button size="sm" variant="light" onPress={() => handleEdit(purchase)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                                                ...
                                                <Button size="sm" variant="light" color="danger" onPress={() => handleDelete(purchase)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
                            ...
                            {paginatedPurchases.map((purchase) => (
                                ...
                                >
                                    <Card>
                                        ...
                                        <CardBody>
                                            <p>Proveedor: {purchase.provider.id}</p>
                                            <p>Fecha: {new Date(purchase.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }).replace(/ (\w+)/, (match, month) => ' ' + month.charAt(0).toUpperCase() + month.slice(1))}</p>
