import { Divider, Tab, Tabs, Button, Card, CardBody } from "@heroui/react";
...
const ProductGrid = ({
    ...
    return (
        <div className="bg-black/10 p-2 rounded-lg max-h-70">
            ...
            {categoriesLoading && <span>Cargando categorías...</span>}
            {categoriesError && <span className="text-red-500">{categoriesError}</span>}
            {productLoading && <span>Cargando productos...</span>}
            {productError && <span className="text-red-500">{productError}</span>}
            <div className="flex flex-col overflow-y-auto gap-0 mt-2 md:flex-row md:flex-wrap md:max-h-54" style={{
                ...
                {products.map((product) => (
                    <Card key={product.id} className="w-full rounded-2xl bg-black/10 px-2 py-[0.25rem] mt-1">
                                ...
                                <div className="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    <div className="flex items-center gap-1">
                                        {productQuantities[product.id] > 0 && (
                                            <Button
                                                size="sm"
                                                className="bg-black text-white w-6 h-6 rounded-full text-sm min-w-0 p-0"
                                    ...
                                    <Button
                                        size="sm"
                                        variant="light"
                                        className="hover:text-white rounded-md p-2 min-w-0"
                                        onPress={() => {
