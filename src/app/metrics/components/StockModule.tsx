...
import { getStockByDay } from "../../services/metrics.service";
import { StockByDay, DateRange } from "../../types/metrics.type";
...
const StockModule = ({ dateRange, refreshKey }: { dateRange: DateRange; refreshKey: number }) => {
  const [stockData, setStockData] = useState<StockByDay[]>([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const data = await getStockByDay(dateRange, ingredientFilter);
        setStockData(data);
      } catch (error) {
  ...
  const uniqueIngredients = Array.from(
    new Set(stockData.map(item => `${item.ingredientId}-${item.ingredientName}`))
  ).map(ingredientStr => {
  ...
  const chartData = {
    labels: Array.from(new Set(stockData.map(item => item.day))).sort(),
    datasets: uniqueIngredients.map((ingredient, index) => {
      const ingredientData = stockData.filter(item => item.ingredientId === ingredient.id);
      const colors = [
        'rgba(255, 99, 132, 1)',
        'rgba(54, 162, 235, 1)',
        'rgba(255, 205, 86, 1)',
      ...
      return {
        ...
        data: Array.from(new Set(stockData.map(item => item.day))).sort().map(day => {
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Stock por ingrediente por día'
      }
    },
    scales: {
      y: {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <Tooltip content="Módulo para visualizar el stock de ingredientes por día">
      ...
      <Card>
        <CardHeader className="flex items-center gap-2">
          <h3 className="text-lg font-semibold">Stock por ingrediente por día</h3>
          <Tooltip content="Gráfico de líneas que muestra la evolución del stock de diferentes ingredientes">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5 cursor-help">
              <path strokeLinecap="round" strokeLinejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
            </svg>
          </Tooltip>
